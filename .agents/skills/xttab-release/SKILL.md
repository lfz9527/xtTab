...
**版本日志面向用户**：记录用户可感知的功能变化，不是开发者视角的提交记录。条目应描述"用户能用什么 / 行为发生了什么变化"：

- 功能类以"新增 / 支持"开头说明用户获得的能力，如"新增新标签页底部常用网站入口栏，一键直达"
- 修复类以"修复……的问题"开头说明用户侧现象，如"修复顶部导航栏视图切换按钮拖拽排序后选中状态异常的问题"
- 不写实现细节：内部变量名、组件名、storage/fallback、文件或函数名、commit 要点等一律不出现

同一变更的正反写法：

- 错误（开发者视角）：移除 useDockItems 中 50 个演示入口及辅助函数，storage fallback 改为空列表
- 正确（用户视角）：未配置常用入口时自动隐藏入口栏
